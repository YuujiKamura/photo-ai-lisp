...
<body>
  ...
  <script type=\"module\">
    ...
    // Forward IME composition events to the parent frame so the
    // sidebar debug log can render them in-place (no extra HUD in
    // the terminal area). Parent listens for { type: 'ime', kind,
    // data, utf8 } on window.message. See static/index.html.
    function imeReport(kind, data) {
      const text = data || '';
      let hex = '';
      if (text) {
      ...
      try {
        window.parent.postMessage(
          { type: 'ime', kind: kind, data: text, utf8: hex },
          '*'
        );
      } catch (_) { /* standalone /shell load — no parent, ignore */ }
    }
