...
(defpreset "コマンド棚卸し"
  :agent "claude"
  :group nil
  :input "サイドバーのプリセット群を **読み取り専用で** 棚卸し・品質点検しろ。
このボタンは read-only audit だ。mutation API (POST /api/presets/new|rewrite|delete|deploy) を **絶対に叩くな**。
書き換え提案は全て Markdown の patch 形式で出力して、ユーザーが
別途チャットで明示的に『当てろ』と言った時だけ適用しろ。
...
5. 修正提案は以下の Markdown patch 形式で列挙するだけ:
     ### [preset name] の修正案
     **before:**
     ```
     現在の :input テキスト
     ...
     **after:**
     ...
     ```
     **理由:** 1-2 行
   複数提案があれば全部並べる。適用コマンドは書かない。

6. 実地の解析実績 (過去の result.json や写真帳出力) と照らして、
   『この preset はこう書いた方が成功率が上がる』という学びがあれば
   併せてレポートしろ。

7. レポートの最後に一言:
     『気に入った修正があれば個別に指示してくれ。こちらから API は
      叩かない。ユーザーが手で curl するか、追加で指示を出してくれ
      れば適用する』
   と添えて終われ。")
...
(defun %presets-live-path ()
  (merge-pathnames "src/presets-live.lisp" (uiop:getcwd)))

(defun %presets-live-backup-path ()
  (merge-pathnames "src/presets-live.lisp.bak" (uiop:getcwd)))
...
(defun deploy-presets ()
  "Serialise the current live state to src/presets-live.lisp. The file
   ...
   survives restart. Before overwriting, copy the existing live file
   to .bak so a bad deploy can be undone via revert-presets (the
   /api/presets/revert endpoint). Returns (:ok :path P :count N)."
  (let ((path (%presets-live-path))
        (bak  (%presets-live-backup-path))
        (names (list-preset-names)))
    ;; Snapshot the pre-deploy file so a later revert can restore it.
    ;; First deploy after a fresh checkout has no file yet — skip.
    ...
    (with-open-file (stream path :direction :output
      ...
      (format stream ";;; rewrites this file in full. Previous version is preserved~%")
      (format stream ";;; at presets-live.lisp.bak (POST /api/presets/revert to restore).~%")
      (format stream ";;; Delete both files to fall back to the factory bundle.~%~%")
...
(defun revert-presets ()
  "Restore src/presets-live.lisp from the .bak snapshot written by
   the last deploy, then hot-reload :presets-live so the in-memory
   registry swaps back atomically. Errors if no backup exists
   (nothing to revert to)."
  (let ((path (%presets-live-path))
        (bak  (%presets-live-backup-path)))
    (unless (uiop:file-exists-p bak)
      (error "no backup to revert to (~a not found)" bak))
    (uiop:copy-file bak path)
    (load path)
    (%notify-presets-changed)
    (list :ok t :restored (namestring path))))
...
(defun preset-deploy-handler ()
  "Body of POST /api/presets/deploy. Writes presets-live.lisp after
   snapshotting the previous version to .bak."
  (handler-case
...
(defun preset-revert-handler ()
  "Body of POST /api/presets/revert. Restores presets-live.lisp from
   the .bak snapshot written by the last deploy, then reloads it so
   the in-memory registry rewinds. 404 if no backup is available."
  (handler-case
      (let ((r (revert-presets)))
        (format nil "{\"ok\":true,\"restored\":~a}"
                (%escape-lisp-string (getf r :restored))))
    (error (e)
      (%preset-error-response 404 (princ-to-string e)))))
...
(defun %presets-dispatch ()
  "Prefix dispatcher for /api/presets[/<verb>[/<name>]].
   The first path segment after /api/presets is the verb:
     ...
     POST /api/presets/revert          → preset-revert-handler
      ...
      (cond
        ...
        ((eq method :post)
         (cond
           ...
           ((equal verb "revert")             (preset-revert-handler))
